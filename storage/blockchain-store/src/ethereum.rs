...
/// 以太坊存储实现
pub struct EthereumStorage {
    web3: Web3<Http>,
    network_config: NetworkConfig,
    contract_address: Option<Address>,
}

impl EthereumStorage {
    /// 创建新的以太坊存储实例
    pub async fn new(network_config: NetworkConfig) -> Result<Self> {
        ...
        tracing::info!("Connected to Ethereum network: {}, Chain ID: {}", network_config.name, chain_id);
    ...
    async fn store_via_contract(&self, key: &str, data: &[u8]) -> Result<StorageTransaction> {
        ...
        let tx_hash = format!("0x{}", hex::encode(&Sha256::digest(data)[..32]));
...
#[async_trait]
impl BlockchainStorage for EthereumStorage {
    async fn store_data(
        &self,
        key: &str,
        data: &[u8],
        ...
        if data.len() > self.network_config.gas_limit.unwrap_or(21000) as usize {
    ...
    async fn delete_data(&self, _key: &str) -> Result<StorageTransaction> {
        // 以太坊不支持删除数据，只能标记为删除
        Err(BlockchainError::Unknown("Delete operation not supported on Ethereum".to_string()))
    }
...
#[async_trait]
impl BlockchainClient for EthereumStorage {
