...
/// Injective 存储实现
pub struct InjectiveStorage {
    network_config: NetworkConfig,
    contract_address: Option<Addr>,
    // 实际实现中需要添加 Injective SDK 客户端
}

impl InjectiveStorage {
    /// 创建新的 Injective 存储实例
    pub async fn new(network_config: NetworkConfig) -> Result<Self> {
        tracing::info!("Connected to Injective network: {}", network_config.name);
    ...
    async fn store_via_contract(&self, key: &str, data: &[u8]) -> Result<StorageTransaction> {
        ...
        let tx_hash = format!("injective_{}", hex::encode(&Sha256::digest(data)[..32]));
        
        Ok(StorageTransaction {
            tx_hash,
            block_number: Some(12345),
            gas_used: Some(180000), // Injective gas
...
#[async_trait]
impl BlockchainStorage for InjectiveStorage {
    async fn store_data(
        &self,
        key: &str,
        data: &[u8],
    ...
    async fn delete_data(&self, _key: &str) -> Result<StorageTransaction> {
        // Injective 不支持删除数据，只能标记为删除
        Err(BlockchainError::Unknown("Delete operation not supported on Injective".to_string()))
    }

    async fn get_stats(&self) -> Result<StorageStats> {
        // 简化实现，实际应该从 Injective 查询统计信息
        Ok(StorageStats {
...
#[async_trait]
impl BlockchainClient for InjectiveStorage {
    async fn connect(&mut self) -> Result<()> {
        // 测试连接
        tracing::info!("Connected to Injective network: {}", self.network_config.name);
        Ok(())
    }

    async fn disconnect(&mut self) -> Result<()> {
        // Injective 客户端不需要显式断开
        Ok(())
    }
